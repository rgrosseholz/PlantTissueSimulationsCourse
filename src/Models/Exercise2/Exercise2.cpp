...
void Exercise2::OnDivide(ParentInfo *parent_info, CellBase *daughter1, CellBase *daughter2)
{ }

void Exercise2::SetCellColor(CellBase *c, QColor *color)
{   // Green: Auxin = growth hormone
  color->setRgb(c->Chemical(1)/(1+c->Chemical(1)) * 255.,(c->Chemical(0)/(1+c->Chemical(0)) * 255.), 0);
}

void Exercise2::CellHouseKeeping(CellBase *c)
{ // nothing is happening here
}

void Exercise2::CelltoCellTransport(Wall *w, double *dchem_c1, double *dchem_c2)
{
  // leaf edge is const source of auxin
  // we need to have some boundary conditions for the signalling hormone
...
void Exercise2::WallDynamics(Wall *w, double *dw1, double *dw2)
{
...
double Exercise2::complex_PijAj(CellBase *here, CellBase *nb, Wall *w)
{ 
...
void Exercise2::CellDynamics(CellBase *c, double *dchem)
{
  ...
  dPidt = -par->k1 * c->ReduceCellAndWalls<double>( far_3_arg_mem_fun( *this, &Exercise2::complex_PijAj ) ) + par->k2 * sum_Pij;
